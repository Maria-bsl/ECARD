...
namespace FUNDING.Controllers
{
  public class CustomerUsersController : Controller
  {
    ...
    public ActionResult IndexDataTable()
    {
      ...
      var cust_users = this._dbContext.cust_users.Where(c => c.cust_reg_sno == eventAdminID && c.event_det_sno.Contains(eventID)).Select(c => new
      {
        c.cust_users_sno,
        c.user_fullname,
        c.user_type,
      ...
      return (ActionResult) this.Json((object) new
      {
        data = cust_users.Select(c => new
        {
          ...
          c.email_address,
          c.mobile_no
        }).OrderByDescending(x => x.cust_users_sno),
        eventAdminID
      });
    }
