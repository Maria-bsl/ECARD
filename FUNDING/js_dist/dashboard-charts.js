...
function stackedChart_multiDatasets(labels, sponsor, sponsorshipValue) {
  ...
  if (disbursementStackedBarChart) {
...
function barChart_teamDisbursement(labels, sponsorshipValue) {
  datasetValue = [{
    backgroundColor: backgroundColor,
    hoverBackgroundColor: hoverBackgroundColor,
    data: sponsorshipValue,
  ...
  if (teamDisbursementBarChart) {
    chartUpdate_mutliDataset_teamDisbursement(
      teamDisbursement_bar_chart_data,
      labels,
      datasetValue,
      teamDisbursementBarChart
    );
  }
}
...
function chartUpdate_mutliDataset_teamDisbursement(chart_data, labels, datasets, Chart) {
  chart_data.labels = labels;
  chart_data.datasets = datasets;
  Chart.options.scales.xAxes[0].scaleLabel.labelString = "Sponsors";
  Chart.update();
}
...
if (isElementExists("balance_pie_chart")) {
...
if (isElementExists("team_expenses_pie_chart")) {
...
if (isElementExists("sponsorship_pie_chart")) {
...
if (isElementExists("disbursement_stacked_bar_chart")) {
...
if (isElementExists("expenses_bar_chart")) {
...
var teamDisbursement_bar_chart_data = {
  datasets: [{
    borderWidth: 0.25,
    weight: 0.8,
    backgroundColor: backgroundColor,
...
if (isElementExists("disbursement_bar_chart")) {
  var teamDisbursementBarChart = chartInit(
    "disbursement_bar_chart",
    teamDisbursement_bar_chart_data,
    "bar"
  );
}
